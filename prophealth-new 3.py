...
# <<< MODIFIED: Property dataclass to include documents >>>
@dataclass
class Property:
    address: str
    city: str
    ...
    documents: List[Dict[str, Any]] = field(default_factory=list) # Stores {'name': str, 'data': bytes, 'type': str, 'upload_time': datetime}

@dataclass
class MaintenanceRecord:
    # ... (MaintenanceRecord dataclass remains the same for now) ...
    date: str
    category: str
    description: str
    cost: float
...
class Dashboard:
    ...
    def render_header(self):
        ...
        st.markdown("Your partner in proactive property care. *Beta Preview v0.9*") # Version bump for Docs
        st.markdown("---")
    ...
    def render_add_property_form_main_panel(self):
        # ... (render_add_property_form_main_panel remains same) ...
                ...
                if submitted:
                    if not all([address, city, state, zip_code]):
                        ...
                        new_property = Property(
                            ...
                            last_inspection=last_inspection_str, image_data=image_bytes, documents=[] # Ensure new prop has empty doc list
                        )
    ...
    def render_add_maintenance_form_main_panel(self):
        # ... (render_add_maintenance_form_main_panel remains same) ...
    ...
    def render_edit_property_form_main_panel(self):
        # ... (render_edit_property_form_main_panel remains same) ...
    ...
    # <<< NEW: Method to render Property Documents section >>>
    def render_property_documents(self, property_data: Property):
        with st.container(border=True):
            st.subheader("📎 Property Documents")
            
            # Find the property index to update it directly
            prop_idx = -1
            for i, p in enumerate(st.session_state.properties):
                if p.address == property_data.address: # Assuming address is unique identifier for now
                    prop_idx = i
                    break
            
            if prop_idx == -1: # Should not happen if property_data is valid
            ...
            uploaded_files = st.file_uploader(
                ...
                key=f"doc_uploader_{property_data.address.replace(' ','_')}" # Unique key
            )
            ...
            if uploaded_files:
                ...
                for uploaded_file in uploaded_files:
                    file_details = {
                        ...
                        "data": uploaded_file.getvalue(), # Read file bytes
                ...
                if files_added_count > 0:
                    ...
                    st.rerun() # Rerun to clear uploader and update list
            ...
            if not st.session_state.properties[prop_idx].documents:
                ...
                for i, doc in enumerate(st.session_state.properties[prop_idx].documents):
                    ...
                    with doc_cols[1]:
                        st.download_button(
                            label="⬇️ Download",
                            data=io.BytesIO(doc['data']), # Use io.BytesIO for data
                            file_name=doc['name'],
                            mime=doc.get('type', 'application/octet-stream'), # Provide MIME type if available
                    ...
                    if i < len(st.session_state.properties[prop_idx].documents) - 1:
                         st.divider() # Use st.divider instead of --- for this context
        st.markdown("---") # Separator after the whole documents block

    def render_health_score_visualization(self, property_data: Property):
        # ... (Content remains same, but now an Edit button is added within this method) ...
        with st.container(border=True):
    ...
    def render_detailed_breakdown(self, property_data: Property):
        # ... (render_detailed_breakdown remains same) ...
        with st.container(border=True): 
            ...
            with property_health_tab:
                ...
                if not st.session_state.properties:
                    ...
                    if selected_property:
                        ...
                        # <<< CALL NEW Document Management Section >>>
