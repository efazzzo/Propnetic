...
st.set_page_config(
    ...
    theme=None # We'll use CSS for more control, but could set base here
)
...
def apply_global_styles():
    st.markdown(f"""
        <style>
            ...
            /* Sidebar background (already styled by Streamlit's secondaryBackgroundColor, 
               but can be overridden if needed. Let's try to use theme settings first) */
            /* .css-1d391kg {{ background-color: {SECONDARY_BACKGROUND}; }} */
            ...
            .st-emotion-cache-10trblm {{ /* Streamlit's default subheader/caption class, might change */
            ...
            .stButton>button[kind="secondary"]:hover, .stButton button.secondary:hover {{
            ...
            .metric-card {{
                padding: 1rem 1.5rem;
                border-radius: 0.5rem; /* More Zillow-like: less rounded */
                color: {TEXT_LIGHT}; /* Default white text */
                margin-bottom: 1rem;
            ...
            .metric-card h3 {{ /* Label */
                color: {TEXT_LIGHT} !important; /* Keep labels light on dark cards */
            ...
            .metric-card p.value {{ /* Value */
                font-size: 2rem; /* Slightly smaller */
                font-weight: bold; /* Bolder value */
                color: {TEXT_LIGHT} !important; /* Ensure value is also light */
            ...
            /* Semantic colors - keeping these for now for clarity, can be discussed */
            .metric-bg-positive {{ background-color: {FOREST_GREEN}; }} /* Forest green for positive */
            .metric-bg-neutral {{ background-color: {PRIMARY_BLUE}; }} /* Blue for neutral/info */
            .metric-bg-warning {{ background-color: #FFC107; }} /* Yellow for warning */
            .metric-bg-warning h3, .metric-bg-warning p.value, .metric-bg-warning small.delta {{ color: #333 !important; }} /* Dark text on yellow */
            .metric-bg-alert {{ background-color: #DC3545; }} /* Red for alert/urgent */
            

            /* --- Containers and Expanders --- */
            .stExpander {{
                border: 1px solid #D3D3D3 !important; /* Lighter border for expanders */
                border-radius: 0.3rem;
            }}
            .stExpander header {{
                background-color: {SECONDARY_BACKGROUND} !important; /* Lighter header for expanders */
                color: {FOREST_GREEN} !important; /* Expander header text */
                border-radius: 0.3rem 0.3rem 0 0;
            }}
            div[data-testid="stVerticalBlock"] div[data-testid="stVerticalBlock"] {{ /* To target bordered containers */
            ...
            .stTabs [data-baseweb="tab-list"] {{
                background-color: {SECONDARY_BACKGROUND}; /* Background for tab bar */
                border-radius: 0.3rem;
                padding: 0.25rem;
            }}
            .stTabs [data-baseweb="tab"] {{
                background-color: transparent;
                color: {PRIMARY_BLUE}; /* Tab text color */
                font-family: 'Times New Roman', Times, serif !important;
            }}
            .stTabs [data-baseweb="tab"][aria-selected="true"] {{
                background-color: {PRIMARY_BLUE}; /* Selected tab background */
                color: {TEXT_LIGHT}; /* Selected tab text color */
                border-radius: 0.3rem;
            }}
            ...
            .stFileUploader > div > div > button, .stNumberInput > div > div > input {{
                border-radius: 0.3rem !important;
                border: 1px solid #CED4DA !important; /* Standard bootstrap border */
                font-family: 'Times New Roman', Times, serif !important;
            }}
            .stFileUploader > label {{ /* Ensure file uploader label also gets font */
                 font-family: 'Times New Roman', Times, serif !important;
                 color: {TEXT_COLOR} !important;
            }}
...
@dataclass
class Property:
    # ... (Property dataclass remains the same) ...
    address: str
    city: str
    state: str
    zip_code: str
...
class MaintenanceRecord:
    # ... (MaintenanceRecord dataclass remains the same) ...
    date: str
    category: str
    description: str
    cost: float
...
def get_weather_data(zip_code: str, api_key: str) -> Optional[Dict[str, Any]]:
    # ... (get_weather_data function remains the same) ...
    if not api_key:
...
class PropertyHealthCalculator:
    # ... (PropertyHealthCalculator remains same, including roof_age logic) ...
    def __init__(self):
    ...
    def generate_maintenance_schedule(self, property_data: Property) -> List[Dict]:
        # ... (generate_maintenance_schedule remains same) ...
        schedule = []
...
class AuthenticationManager:
    ...
    def render_auth_screen(self):
        ...
        .auth-container h1 { color: #764ba2; text-align: center; } /* Keeping original auth screen style */
        ...
        </style>""", unsafe_allow_html=True) # Auth screen style distinct from main app
        ...
        if st.button("🚀 ACCESS PREVIEW", type="primary", disabled=access_button_disabled): # This button will use auth screen style
            if password_correct:
    ...
    def render_session_info(self):
        if self.is_authenticated():
            ...
            # Logout button style will be affected by global styles
            if st.sidebar.button("🚪 End Session & Logout", use_container_width=True, key="logout_button_main_app"): # Use a unique key
...
class Dashboard:
    def __init__(self):
        ...
        # <<< Apply global styles after auth check in run(), or here if auth is simple >>>
        # Moved to run() to ensure it applies only after authentication for main app

    def init_session_state(self):
        # ... (init_session_state remains same) ...
    ...
    def render_header(self):
        st.title("🏠 Property Health Intelligence") # Simplified Title
        # st.markdown("Your partner in proactive property care. *Beta Preview v1.1*") # Version bump for Theming
        st.markdown("---")
        
        # <<< Updated Metric Card rendering to use new CSS classes >>>
        cols = st.columns(5) 
    ...
    def render_quick_actions(self):
        # ... (render_quick_actions remains same functionally) ...
    ...
    # ... (All other render methods for forms, property selector, maintenance history, charts, etc., remain functionally the same)
    # ... (The visual appearance of these will be affected by the global CSS changes)
        ...
        if st.session_state.properties:
            with st.sidebar.expander("🗑️ Manage Properties", expanded=False): 
                ...
                if selected_prop_display_label_to_delete and selected_prop_display_label_to_delete != "":
                    ...
                    # Use a class for this button if needed for specific styling not covered by general .stButton
    ...
    def render_add_property_form_main_panel(self):
        ...
        with st.container(border=True):
            with st.form("property_form_main", clear_on_submit=False): 
                ...
                with cancel_col:
                    # Add kind="secondary" or a specific class if you want the blue outline style from CSS
                    if st.form_submit_button("❌ Cancel", use_container_width=True, type="secondary"): # Using type="secondary" for Streamlit's own secondary
                        st.session_state.show_add_property_form_main = False
                        st.rerun()
    ...
    def render_add_maintenance_form_main_panel(self):
        ...
        if not st.session_state.properties:
            ...
            if st.button("Go to Add Property"): # This button will get global styling
                st.session_state.show_add_maintenance_form_main = False
                st.session_state.show_add_property_form_main = True
                st.rerun()
            return
        ...
        with st.container(border=True):
            ...
            with st.form("maintenance_form_main", clear_on_submit=False):
                ...
                with cancel_col:
                    if st.form_submit_button("❌ Cancel", use_container_width=True, type="secondary"): # Using type="secondary"
                        st.session_state.show_add_maintenance_form_main = False
                        st.rerun()
    ...
    def render_property_documents(self, property_data: Property):
        # ... (render_property_documents remains same) ...
        with st.container(border=True): # This will now get the border from global CSS
            ...
            if not st.session_state.properties[prop_idx].documents:
                ...
                for i, doc in enumerate(st.session_state.properties[prop_idx].documents):
                    ...
                    with doc_cols[1]:
                        # Download button will use global style
                        st.download_button(
                            label="⬇️ Download",
                    ...
                    with doc_cols[2]:
                        # Delete button will use global style (or secondary if we make that distinct)
    ...
    def render_health_score_visualization(self, property_data: Property):
        # ... (render_health_score_visualization remains same, Edit button added) ...
        with st.container(border=True):
            ...
            with cols_main[1]:
                ...
                st.metric("Overall Health Score", f"{overall_score_val:.1f} / 100") # This metric widget has its own styling

                if overall_score_val >= 85: st.success("Excellent Condition! 👍") # These will use Streamlit's default alert styles
    ...
    def render_weather_display(self, property_zip: str, property_address_display: str):
        # ... (render_weather_display remains same) ...
        with st.container(border=True): 
    ...
    def render_detailed_breakdown(self, property_data: Property):
        # ... (render_detailed_breakdown remains same, will include roof age caption) ...
        with st.container(border=True): 
            ...
            for category, data in scores['category_scores'].items():
                with st.expander(f"{category} Details - Overall Score: {data['score']:.1f}", expanded=(category=="Structural")): 
                    ...
                    comp_cols = st.columns(num_components if num_components > 0 and num_components <= 3 else 1) # Max 3 for readability
                    idx = 0
                    for component, score_val_raw in data['components'].items():
                        ...
                        with current_col:
                            ...
                            if c_score >= 80: color = FOREST_GREEN # Use new theme color
                            elif c_score >= 60: color = PRIMARY_BLUE # Use new theme color
                            else: color = "#DC3545" # Keep red for low scores for now
    ...
    def render_predictions_and_recommendations(self, property_data: Property):
        # ... (render_predictions_and_recommendations remains same) ...
        with st.container(border=True): 
            st.subheader("💡 Predictions & Proactive Recommendations")
            schedule = self.calculator.generate_maintenance_schedule(property_data)
            tab1, tab2 = st.tabs(["🔮 Upcoming Maintenance Tasks", "💰 Cost Planning Tool"]) # Tabs will pick up new styling
            with tab1:
    ...
    def render_property_selector(self, key_suffix="main", allow_none=False, help_text="Select Property to View Details:"):
        # ... (render_property_selector remains same) ...
        if not st.session_state.properties:
    ...
    def render_maintenance_history(self, property_address: Optional[str]):
        # ... (render_maintenance_history remains same) ...
    ...
    def render_comparative_health_chart(self):
        # ... (render_comparative_health_chart remains same) ...
        if len(st.session_state.properties) > 1:
            ...
            if score_types_to_plot:
                ...
                st.bar_chart(chart_df, height=400) # Bar chart colors will be Streamlit's default, can be customized if needed
            else:
    ...
    def render_contractor_marketplace(self):
        # ... (render_contractor_marketplace remains same) ...
    ...
    def render_roi_calculator(self):
        # ... (render_roi_calculator remains same) ...
    ...
    def run(self):
        ...
        apply_global_styles() # <<< Apply new global styles after authentication
        self.render_header()

        # ... (rest of the run method, sidebar and main panel form/tab logic remains same) ...
        with st.sidebar:
            ...
            st.markdown("---")
            st.sidebar.caption(f"© {datetime.datetime.now().year} Property Health Intel.") # Removed All Rights Reserved for brevity
        ...
        if st.session_state.get('editing_property_address') is not None:
            ...
            # Using st.session_state to manage active tab for persistence
            # active_tab_idx = tab_options.index(st.session_state.active_tab)
            # selected_tab_title = st.tabs(tab_options)[active_tab_idx] # This doesn't work directly to set active
            
            tabs_tuple = st.tabs(tab_options) # This creates the tabs
            
            # Logic to handle which tab's content to show based on st.session_state.active_tab
            # This part is tricky with st.tabs() as it doesn't directly support programmatic control of active tab *after* creation.
            # The current approach of just rendering all and letting Streamlit handle it is fine.
            # If we needed to force a tab change from code, we'd typically set st.session_state.active_tab and rerun.
            
            property_health_tab, maintenance_tab, contractors_tab, analytics_tab, roi_tab = tabs_tuple

            with property_health_tab:
                # st.subheader("Property Health Dashboard") # Using st.header now for main page titles
