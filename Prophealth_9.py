...
def get_weather_data(zip_code: str, api_key: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if response.status_code == 401: # type: ignore
    ...
    def render_session_info(self):
        if self.is_authenticated():
            ...
            if st.sidebar.button("🚪 End Session & Logout", type="secondary", use_container_width=True):
                ...
                # Note: 'openweathermap_api_key' is no longer in session_state if using st.secrets
...
class Dashboard:
    ...
    def init_session_state(self):
        ...
        # No need to initialize openweathermap_api_key in session_state if using st.secrets

    def render_header(self):
        # ... (render_header method remains the same) ...
        st.title("🏠 Property Health Intelligence Dashboard")
        st.markdown("Your partner in proactive property care. *Beta Preview v0.5.1*") # Version bump for st.secrets
        st.markdown("---")
        cols = st.columns(5) 
    ...
    def render_weather_display(self, property_zip: str, property_address: str):
        # ... (content changed to use st.secrets) ...
        ...
        if weather_data:
            ...
            # This case might be redundant if get_weather_data always returns a dict with "error"
    ...
    def render_roi_calculator(self):
        # ... (method remains the same as previously provided) ...
    ...
    def run(self):
        ...
        with st.sidebar:
            ...
            # <<< REMOVED API KEY INPUT FROM SIDEBAR - NOW USES st.secrets >>>
            # If you still want a visual indicator or settings area, you can add a small note:
            with st.expander("⚙️ App Settings", expanded=False):
                st.caption("Weather API key is configured via Streamlit Secrets.")
                # You could add other app-wide settings here in the future.
