class AuthenticationManager:
    ...
    def render_auth_screen(self):
        ...
        with col1:
            # Widgets automatically update st.session_state using their keys.
            # The variable 'confidential_understood' will hold the current state.
        ...
        # Assign keys to all input widgets for robust state management
        ...
        password_input = st.text_input("Enter access code:", type="password", placeholder="Access code required", key="auth_password_input")

        # Retrieve values from session state for logic
        # For checkboxes, the variables like 'confidential_understood' already hold their current boolean state.
        ...
        s_password_input = st.session_state.get("auth_password_input", "") # Get password from session_state as well
        ...
        # THIS BLOCK WAS THE LIKELY CAUSE OF THE ERROR AND IS NO LONGER NEEDED
        # because widgets with keys automatically update st.session_state.
        #
        # # Store input values in session state to persist them on interactions
        # st.session_state.conf_checkbox = confidential_understood # Problematic line
        ...
        # st.session_state.full_name = full_name # Problematic if full_name widget gets a key
    ...
    def render_session_info(self):
        if self.is_authenticated():
            ...
            if st.sidebar.button("🚪 End Session & Logout", type="secondary", use_container_width=True):
                # Selectively clear session state if needed, or clear most of it
                # For simplicity, we can clear keys related to auth form and session info
                ...
                for key in auth_keys_to_clear + other_session_keys:
                    if key in st.session_state:
                        del st.session_state[key]
                # Decide if you want to clear 'properties', 'maintenance_records' on logout
                # e.g., del st.session_state.properties; del st.session_state.maintenance_records
