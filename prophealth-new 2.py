...
import numpy as np
import datetime
from dataclasses import dataclass, field, asdict # Added asdict for pre-filling edit form easily
...
class Property:
    ...
    electrical_age: int
    plumbing_age: int
    last_inspection: str # Stored as "YYYY-MM-DD" or "N/A"
        ...
        if self.is_authenticated():
            ...
            if st.sidebar.button("🚪 End Session & Logout", type="secondary", use_container_width=True):
                ...
                keys_to_clear.extend(['access_info', 'authenticated', 'access_timestamp', 
                                      'show_add_property_form_main', 'show_add_maintenance_form_main', # Updated flags
                                      'editing_property_address', # <<< NEW: Clear edit flag on logout
...
class Dashboard:
    ...
    def init_session_state(self):
        ...
        # <<< NEW: State for managing property editing >>>
        if 'editing_property_address' not in st.session_state:
            st.session_state.editing_property_address = None # Stores address of property being edited
    ...
    def render_header(self):
        ...
        st.markdown("Your partner in proactive property care. *Beta Preview v0.8*") # Version bump for Edit Property
        st.markdown("---")
    ...
    def render_quick_actions(self):
        ...
        if st.sidebar.button("➕ Add New Property", use_container_width=True, key="quick_add_prop_btn_main"):
            ...
            st.session_state.show_add_maintenance_form_main = False 
            st.session_state.editing_property_address = None # <<< Close edit form
            st.rerun()
                ...
                if selected_prop_display_label_to_delete and selected_prop_display_label_to_delete != "":
                    ...
                    if st.button("🗑️ Delete Selected Property", type="secondary", use_container_width=True, key="delete_prop_btn_sidebar"):
                        ...
                        if idx_to_delete != -1:
                            ...
                            # If the deleted property was being edited or selected, clear those states
                            ...
                            if st.session_state.get(f"selected_property_address_health") == address_to_delete:
                                st.session_state.selected_property_address_health = None # Or select first if available
    ...
    def render_add_property_form_main_panel(self):
        # ... (render_add_property_form_main_panel remains same as previous version) ...
    ...
    def render_add_maintenance_form_main_panel(self):
        # ... (render_add_maintenance_form_main_panel remains same as previous version) ...
    ...
    # <<< NEW: Method to render Edit Property form in MAIN PANEL >>>
    def render_edit_property_form_main_panel(self):
        ...
        with st.container(border=True):
            # Use default values from prop_to_edit for the form
            # Need to handle date conversion for last_inspection if it's not "N/A"
            ...
            if prop_to_edit.last_inspection and prop_to_edit.last_inspection != "N/A":
                try:
                    ...
                    default_last_inspection = None # Keep as None if parsing fails
            ...
            with st.form(f"edit_property_form_{prop_to_edit.address.replace(' ','_')}", clear_on_submit=False):
                ...
                with col2:
                    ...
                    roof_material_options = ["Asphalt Shingles", "Metal", "Tile", "Slate", "Wood", "Composite", "Flat Roof (TPO/EPDM)"]
                    ...
                    foundation_type_options = ["Concrete Slab", "Basement", "Crawl Space", "Pier & Beam"]
                ...
                # For image, we can't easily pre-fill st.file_uploader. Show current, allow replacement.
                ...
                if submitted:
                    if not all([address, city, state, zip_code]):
                        ...
                        # Update the property object directly
    ...
    def render_health_score_visualization(self, property_data: Property):
        ...
        with st.container(border=True):
            ...
            if st.button("✏️ Edit This Property", key=f"edit_prop_{property_data.address.replace(' ','_')}"):
                ...
                st.session_state.show_add_property_form_main = False # Close other forms
                st.session_state.show_add_maintenance_form_main = False
                st.rerun()
    ...
    def render_weather_display(self, property_zip: str, property_address_display: str):
        ...
        with st.container(border=True): 
            ...
            # ... (rest of weather logic remains same) ...
    ...
    def render_detailed_breakdown(self, property_data: Property):
        ...
        with st.container(border=True): 
            ...
            # ... (rest of detailed breakdown logic remains same) ...
            scores = self.calculator.calculate_overall_score(property_data)
            for category, data in scores['category_scores'].items():
                with st.expander(f"{category} Details - Overall Score: {data['score']:.1f}", expanded=(category=="Structural")): 
                    ...
                    comp_cols = st.columns(num_components if num_components > 0 and num_components <= 4 else 1) 
                    idx = 0
    ...
    def render_predictions_and_recommendations(self, property_data: Property):
        ...
        with st.container(border=True): 
            ...
            # ... (rest of predictions logic remains same) ...
    ...
    def render_property_selector(self, key_suffix="main", allow_none=False, help_text="Select Property to View Details:"):
        # ... (render_property_selector remains the same as previous version with city/state display) ...
        if not st.session_state.properties:
    ...
    def render_maintenance_history(self, property_address: Optional[str]):
        # ... (render_maintenance_history remains the same as previous version with city/state display) ...
    ...
    def render_comparative_health_chart(self):
        # ... (render_comparative_health_chart remains the same as previous version) ...
    ...
    def render_contractor_marketplace(self):
        # ... (render_contractor_marketplace remains the same as previous version) ...
    ...
    def render_roi_calculator(self):
        # ... (render_roi_calculator remains the same as previous version) ...
    ...
    def run(self):
        ...
        with st.sidebar:
            ...
            st.markdown("---")
            self.render_property_management_sidebar() # Manages delete property
            st.markdown("---")
            st.sidebar.header("🛠️ Maintenance Log") 
            st.sidebar.caption("Use Quick Action button to log.") # Minimal text, form moved
            st.markdown("---")
            st.sidebar.caption(f"© {datetime.datetime.now().year} Property Health Intel. All Rights Reserved.")

        # <<< Main panel: Show a form OR the tabs >>>
            ...
            with property_health_tab:
                ...
                if not st.session_state.properties:
                    ...
                    if selected_property:
                        # These are now wrapped in containers with borders within their respective render methods
