#!/usr/bin/env python3
# Prophealth_3.py - Integrated with Tenant Tools and Digital Binder
...
def apply_global_styles():
    st.markdown(f"""
        <style>
            ...
            /* Metric card styling for header */
            .metric-card {{
                padding: 15px;
                border-radius: 10px;
                color: white;
            ...
            .metric-bg-warning {{ background-color: #FFC107; }} /* Yellow */
            .metric-bg-alert {{ background-color: #DC3545; }}   /* Red */
            .metric-bg-neutral {{ background-color: #6C757D; }}  /* Gray */
        </style>
...
class AuthenticationManager:
    ...
    def render_auth_screen(self):
        ...
        .auth-container h4 { color: #555; margin-top:1rem; margin-bottom:0.2rem; }
...
class Dashboard:
    ...
    def render_admin_tenant_management(self):
        ...
        if not self.auth_manager.is_admin():
            st.warning("🔒 You must be logged in as an administrator to access this section.")
            return
        ...
        if st.session_state.get('editing_property_address') is not None:
            ...
            with tabs_tuple[2]: # NEW: Tenant Services Tab
                ...
                with tenant_sub_tabs[1]: # Tenant Management (Admin Only)
                    if self.auth_manager.is_admin():
                        self.render_admin_tenant_management() # CALL THE NEW METHOD HERE
                    else:
                        st.warning("🔒 You need administrator privileges to access tenant management features. Please log in as an Admin.")

                with tenant_sub_tabs[2]: # Community Board (Verified Tenants)
                    self.render_community_board() # CALL THE NEW METHOD HERE
