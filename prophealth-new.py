...
@dataclass
class Property:
    # ... (Property dataclass remains the same, including city/state) ...
    address: str
    city: str
    state: str
    zip_code: str
...
class PropertyHealthCalculator:
    ...
    def generate_maintenance_schedule(self, property_data: Property) -> List[Dict]:
        # ... (generate_maintenance_schedule remains the same) ...
        schedule = []
...
class AuthenticationManager:
    # ... (AuthenticationManager class remains the same) ...
    def __init__(self):
        ...
        if self.is_authenticated():
            ...
            if st.sidebar.button("🚪 End Session & Logout", type="secondary", use_container_width=True):
                ...
                keys_to_clear.extend(['access_info', 'authenticated', 'access_timestamp', 
                                      'show_add_property_form', 'show_add_maintenance_form', 'active_tab', # These form flags still used
...
class Dashboard:
    ...
    def init_session_state(self):
        ...
        # <<< Flags now control main panel form visibility >>>
        if 'show_add_property_form_main' not in st.session_state: # Renamed for clarity
            st.session_state.show_add_property_form_main = False 
        if 'show_add_maintenance_form_main' not in st.session_state: # Renamed for clarity
    ...
    def render_header(self):
        ...
        st.markdown("Your partner in proactive property care. *Beta Preview v0.7*") # Version bump
        st.markdown("---")
    ...
    # <<< MODIFIED: render_quick_actions to set main panel form flags >>>
    def render_quick_actions(self):
        ...
        if st.sidebar.button("➕ Add New Property", use_container_width=True, key="quick_add_prop_btn_main"):
            ...
            st.session_state.show_add_maintenance_form_main = False # Ensure other form is closed
            st.rerun()
        ...
        if st.sidebar.button("📝 Log Maintenance Task", use_container_width=True, disabled=add_maint_disabled, key="quick_log_maint_btn_main"):
            ...
            st.session_state.show_add_property_form_main = False # Ensure other form is closed
            st.rerun()
        ...
        if st.sidebar.button("❗ View Urgent Tasks", use_container_width=True, disabled=view_urgent_disabled, key="quick_view_urgent_btn"):
            st.session_state.show_add_property_form_main = False # Close any open forms
            st.session_state.show_add_maintenance_form_main = False
    ...
    # <<< NEW: Method to render only sidebar parts of property management (delete) >>>
    def render_property_management_sidebar(self):
        st.sidebar.header("🏡 Property Portfolio")
        if st.session_state.properties:
            with st.sidebar.expander("🗑️ Manage Properties", expanded=False): # Keep delete in sidebar
                ...
                selected_prop_display_label_to_delete = st.selectbox(
                    ...
                    prop_display_labels, 
                    index=0, 
                    key="delete_prop_select_sidebar" # Unique key
                )
    ...
    # <<< NEW: Method to render Add Property form in MAIN PANEL >>>
    def render_add_property_form_main_panel(self):
        ...
        with st.container(border=True):
            with st.form("property_form_main", clear_on_submit=False): # Clear on submit can be tricky with main panel forms
                ...
                submit_col, cancel_col = st.columns([0.8, 0.2]) # Adjust ratio as needed
                with submit_col:
                ...
                if submitted:
                    if not all([address, city, state, zip_code]):
                        ...
                        st.balloons()
                        # Short delay then rerun to allow user to see success message
                        import time; time.sleep(2) 
                        st.rerun()
    
    # <<< NEW: Method to render Add Maintenance form in MAIN PANEL >>>
    ...
    # <<< MODIFIED: render_health_score_visualization for visual refinement >>>
    def render_health_score_visualization(self, property_data: Property):
        ...
        st.markdown("---") # Separator after this whole block

    # <<< MODIFIED: render_weather_display for visual refinement >>>
    def render_weather_display(self, property_zip: str, property_address_display: str):
        with st.container(border=True): # Wrap in container
            ...
            # ... (rest of weather display logic from previous version) ...
            if weather_data:
    ...
    # <<< MODIFIED: render_detailed_breakdown for visual refinement >>>
    def render_detailed_breakdown(self, property_data: Property):
        with st.container(border=True): # Wrap in container
            st.subheader("🔬 Detailed Component Scores")
            scores = self.calculator.calculate_overall_score(property_data)
            # ... (rest of detailed breakdown logic from previous version) ...
            for category, data in scores['category_scores'].items():
                with st.expander(f"{category} Details - Overall Score: {data['score']:.1f}", expanded=(category=="Structural")): # Keep structural expanded by default
                    if not data['components']:
                        st.write(f"No specific components listed for {category}.")
                        continue
                    
                    num_components = len(data['components'])
                    comp_cols = st.columns(num_components if num_components > 0 and num_components <= 4 else 1) # Max 4 cols for sanity
                    idx = 0
                    for component, score_val_raw in data['components'].items():
                        current_col = comp_cols[idx % len(comp_cols)] # Cycle through columns
                        with current_col:
    ...
    def render_predictions_and_recommendations(self, property_data: Property):
        ...
        # No st.markdown("---") here as the container provides separation

    # ... (render_property_selector, render_maintenance_history, render_comparative_health_chart, render_contractor_marketplace, render_roi_calculator remain largely the same as previous full code, but ensure property display strings are updated where they appear)
    # Minor updates for clarity in property display within these methods were already done.
    ...
    def render_roi_calculator(self):
        # ... (render_roi_calculator remains the same, using updated property selector) ...
    ...
    # <<< MODIFIED: run() method for main panel forms and refined Property Health tab >>>
    def run(self):
        if not self.auth_manager.is_authenticated():
            self.auth_manager.render_auth_screen()
            return

        self.render_header() # Render header first
        ...
        with st.sidebar:
            ...
            self.render_quick_actions() # Buttons that set main panel form flags
            st.markdown("---")
            self.render_property_management_sidebar() # Only sidebar part (delete property)
            st.markdown("---")
            st.sidebar.header("🛠️ Maintenance Log") # Header for log maint quick action
        ...
        # <<< Main panel logic: Show form OR tabs >>>
        if st.session_state.get('show_add_property_form_main', False):
            ...
            with property_health_tab:
                ...
                if not st.session_state.properties:
                    ...
                    if selected_property:
                        ...
                        self.render_weather_display(selected_property.zip_code, f"{selected_property.address}, {selected_property.city}") # Moved here
